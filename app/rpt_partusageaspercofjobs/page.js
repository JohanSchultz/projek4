...
import { PartUsageAsPercOfJobsContent } from "./PartUsageAsPercOfJobsContent";
...
async function fetchRptPartUsagePercOfJobs(
  p_mine_id,
  p_shaft_id,
  p_section_id,
  p_gang_id,
  ...
  try {
    ...
    const typeIds = Array.isArray(p_equipmenttypes_id)
      ...
      : [];
    const supabase = await createClient();
    const { data, error } = await supabase.rpc("rpt_partusagepercofjobs", {
      p_mine_id: mineId,
      p_shaft_id: shaftId,
...
export default async function RptPartUsageAsPercOfJobsPage() {
  await requireFunctionAccess(21);
    ...
    <div className="flex min-h-screen flex-col bg-zinc-50 font-sans dark:bg-zinc-950">
      ...
      <main className="flex-1 overflow-auto p-6">
        ...
        <PartUsageAsPercOfJobsContent
          ...
          fetchRptPartUsagePercOfJobs={fetchRptPartUsagePercOfJobs}
        />
      </main>
    </div>
  );
