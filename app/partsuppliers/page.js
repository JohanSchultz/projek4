...
import { PartSuppliersContent } from "./PartSuppliersContent";
...
async function insertPartSupplier(descr, isactive) {
  "use server";
  try {
    const trimmedDescr = typeof descr === "string" ? descr.trim() : "";
    if (!trimmedDescr) {
      return { error: "Part Supplier is required." };
    }
    const supabase = await createClient();
    const { data: existing } = await supabase
      ...
      .maybeSingle();
    if (existing) {
      return { error: "A part supplier with this description already exists." };
    }
    const { error } = await supabase.from("partsuppliers").insert({
      descr: trimmedDescr,
...
async function updatePartSupplier(id, descr, isactive) {
  "use server";
  try {
    const supabase = await createClient();
    const updates = {};
...
async function fetchAllPartSuppliers() {
  "use server";
  try {
    const supabase = await createClient();
...
export default async function PartSuppliersPage() {
  await requireFunctionAccess(15);
  return (
    <div className="flex min-h-screen flex-col bg-zinc-50 font-sans dark:bg-zinc-950">
      ...
      <main className="flex-1 overflow-auto p-6">
        <PartSuppliersContent
          insertPartSupplier={insertPartSupplier}
          updatePartSupplier={updatePartSupplier}
          fetchAllPartSuppliers={fetchAllPartSuppliers}
        />
      </main>
    </div>
  );
