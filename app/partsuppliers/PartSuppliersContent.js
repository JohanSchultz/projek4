...
import { PartSuppliersGrid } from "./PartSuppliersGrid";
...
export function PartSuppliersContent({
  insertPartSupplier,
  updatePartSupplier,
  fetchAllPartSuppliers,
}) {
  ...
  const loadGrid = () => {
    if (typeof fetchAllPartSuppliers !== "function") return;
    fetchAllPartSuppliers().then((res) => {
  ...
  const handleSave = () => {
    ...
    if (!trimmedDescr) {
      setSaveError("Part Supplier is required.");
      return;
    }
    startTransition(async () => {
      const result = await insertPartSupplier(trimmedDescr, isactive);
      if (result?.error) {
  ...
  const handleChange = () => {
    ...
    startTransition(async () => {
      const result = await updatePartSupplier(
        selectedRowId,
        trimmedDescr,
        isactive
      );
    ...
    <>
      <div className="mb-6 flex flex-wrap items-end gap-6">
        ...
        <div className="flex items-center gap-2">
          <input
            type="checkbox"
            id="partsupplier-active"
            checked={isactive}
            onChange={(e) => setIsactive(e.target.checked)}
            className={checkboxClass}
          />
          <label
            htmlFor="partsupplier-active"
      ...
      <hr className="my-4 border-zinc-200 dark:border-zinc-700" />
      <div>
        <PartSuppliersGrid data={gridData} onRowClick={handleRowClick} />
      </div>
    </>
  );
}
