...
export function TechniciansContent({
  ...
  const loadGrid = () => {
    if (typeof fetchAllTechnicians !== "function") return;
    fetchAllTechnicians().then((res) => {
  ...
  const handleSave = () => {
    ...
    if (!trimmedDescr) {
      setSaveError("Technician is required.");
      return;
    }
    startTransition(async () => {
      const result = await insertTechnician(trimmedDescr, isactive);
      if (result?.error) {
  ...
  return (
    <>
      ...
      <hr className="my-4 border-zinc-200 dark:border-zinc-700" />
      <div>
        <TechniciansGrid data={gridData} onRowClick={handleRowClick} />
      </div>
    </>
  );
}
