...
import { TechniciansContent } from "./TechniciansContent";
...
async function insertTechnician(descr, isactive) {
  "use server";
  try {
    ...
    const { data: existing } = await supabase
      ...
      .maybeSingle();
    if (existing) {
      return { error: "A technician with this description already exists." };
    }
...
async function updateTechnician(id, descr, isactive) {
  "use server";
  try {
    const supabase = await createClient();
    const updates = {};
...
export default async function TechniciansPage() {
  await requireFunctionAccess(16);
  return (
    <div className="flex min-h-screen flex-col bg-zinc-50 font-sans dark:bg-zinc-950">
      ...
      <main className="flex-1 overflow-auto p-6">
        <TechniciansContent
          ...
          fetchAllTechnicians={fetchAllTechnicians}
        />
      </main>
    </div>
  );
