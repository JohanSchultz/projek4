...
async function fetchPartStockLevels() {
  "use server";
  try {
    const supabase = await createClient();
    const { data, error } = await supabase.rpc("partstocklevels");
...
async function fetchPartStockLevelsToOrder() {
  "use server";
  try {
    const supabase = await createClient();
    const { data, error } = await supabase.rpc("partstocklevels_to_order");
    if (error) throw error;
    return { data: Array.isArray(data) ? data : [], error: null };
  } catch (err) {
    return { data: null, error: err?.message ?? String(err) };
  ...
  return (
    <div className="flex min-h-screen flex-col bg-zinc-50 font-sans dark:bg-zinc-950">
      ...
      <main className="flex-1 overflow-auto p-6">
        <PartStockLevelsContent
          fetchPartStockLevels={fetchPartStockLevels}
          fetchPartStockLevelsToOrder={fetchPartStockLevelsToOrder}
        />
      </main>
    </div>
  );
