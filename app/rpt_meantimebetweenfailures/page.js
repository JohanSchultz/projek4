...
import { MeanTimeBetweenFailuresContent } from "./MeanTimeBetweenFailuresContent";
...
async function fetchRptMeanTimeBetweenFailures(
  p_mine_id,
  p_shaft_id,
  p_section_id,
  p_gang_id,
  ...
  try {
    ...
    const typeIds = Array.isArray(p_equipmenttypes_id)
      ...
      : [];
    const supabase = await createClient();
    const { data, error } = await supabase.rpc("rpt_meantimebetweenfailures", {
      p_mine_id: mineId,
      p_shaft_id: shaftId,
...
export default async function RptMeanTimeBetweenFailuresPage() {
  await requireFunctionAccess(34);
  ...
  return (
    <div className="flex min-h-screen flex-col bg-zinc-50 font-sans dark:bg-zinc-950">
      ...
      <main className="flex-1 overflow-auto p-6">
        ...
        <MeanTimeBetweenFailuresContent
          ...
          fetchRptMeanTimeBetweenFailures={fetchRptMeanTimeBetweenFailures}
        />
      </main>
    </div>
  );
