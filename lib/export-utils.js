...
/**
 * Export Cost by Area and Type grid to Excel with formatting:
 * - A1: "Cost by Area and Type" merged A1:F1, Aptos 18, centred
 * - Row 2: height 57; image top-left 96x96
 * - A3: "Report Date: " + current date (dd MMM yyyy), merge A3:M3
 * - A4: "From " + fromDate + " To " + toDate, merge A4:M4
 * - A5–A8: Mine, Shaft, Section, Gang, merge A5:F5, A6:F6, A7:F7, A8:F8
 * - A9: "Equipment Types: " + comma-separated list, merge A9:ZZ9
 * - Rows 3–9: Aptos, size 14
 * - Tabular data starts on row 11; font Aptos 11; header bold + yellow; gridlines
 * @param {Array<Object>} groupedRows - Grid data (mine, shaft, section, gang, equipmenttype, cost)
 * @param {string} [filename]
 * @param {string} [sheetName]
 * @param {{ fromDate?: string, toDate?: string, mine?: string, shaft?: string, section?: string, gang?: string, equipmentTypeLabels?: string[] }} [filterOptions]
 */
export async function exportCostByAreaAndTypeToExcel(
  groupedRows,
  filename = "cost_by_area_and_type.xlsx",
  sheetName = "Cost by Area and Type",
  filterOptions = {}
) {
  const headerLabels = ["Mine", "Shaft", "Section", "Gang", "Equipment Type", "Cost"];
  const accessorKeys = ["mine", "shaft", "section", "gang", "equipmenttype", "cost"];
  const dataRows = (Array.isArray(groupedRows) ? groupedRows : []).map((row) =>
    accessorKeys.map((key) => (key === "cost" ? formatCost(row[key]) : (row[key] ?? "")))
  );
  ...
  ws.getRow(1).getCell(1).value = "Cost by Area and Type";
  ws.getRow(1).getCell(1).font = { name: "Aptos", size: 18 };
  ws.getRow(1).getCell(1).alignment = { horizontal: "center", vertical: "middle" };
  ...
  ws.getRow(4).getCell(1).value = `From  ${fromDateStr}  To  ${toDateStr}`;
  ws.getRow(4).getCell(1).font = infoFont;
...
/**
 * Export Part Stock Levels grid to Excel with formatting:
 * - A1: title ("All Parts Stock Level" or "Stock Level of Parts to Re-order"), merge A1:F1, Aptos 18, centred
 * - Row 2: height 57; image top-left 96x96
 * - A3: "Report Date: " + current date (dd MMM yyyy), merge A3:F3, Aptos 14
 * - Tabular data starts on row 5; font Aptos 11; header bold + yellow; gridlines
 * @param {Array<Object>} rows - Grid data (stockcode, matcatno, binno, stocklevel, reorder, supplier)
 * @param {string} title - "All Parts Stock Level" or "Stock Level of Parts to Re-order"
 * @param {string} [filename]
 * @param {string} [sheetName]
 */
export async function exportPartStockLevelsToExcel(
  rows,
  title,
  filename = "part_stock_levels.xlsx",
  sheetName = "Part Stock Levels"
) {
  const headerLabels = ["Stock Code", "Mat Cat No", "Bin No", "Stock Level", "Reorder", "Supplier"];
  const accessorKeys = ["stockcode", "matcatno", "binno", "stocklevel", "reorder", "supplier"];
  const dataRows = (Array.isArray(rows) ? rows : []).map((row) =>
    accessorKeys.map((key) => (row[key] != null ? String(row[key]) : ""))
  );
  const headerRow = headerLabels;
  const colCount = headerRow.length;
  const headerExcelRow = 5;
  ...
  ws.getRow(3).getCell(1).value = `Report Date: ${reportDateStr}`;
  ws.getRow(3).getCell(1).font = { name: "Aptos", size: 14 };
...
/**
 * Export Average Part Usage Per Month grid to Excel with formatting:
 * - A1: "Average Part Usage Per Month", merge A1:H1, Aptos 18, centred
 * - Row 2: height 57; image top-left 96x96
 * - A3: "Report Date: " + current date (dd MMM yyyy), merge A3:H3, Aptos 14
 * - A4: "From " + fromDate + " To " + toDate, merge A4:H4
 * - A5-A8: Mine, Shaft, Section, Gang; A9: Equipment Types (merge A9:ZZ9)
 * - Rows 3-9: Aptos 14
 * - Tabular data starts on row 11; font Aptos 11; header bold + yellow; gridlines
 * @param {Array<Object>} rows - Grid data (stockcode, matcatno, part, unitcost, qty_used, totalcost, costpermonth, qtypermonth)
 * @param {{ fromDate?: string, toDate?: string, mine?: string, shaft?: string, section?: string, gang?: string, equipmentTypeLabels?: string[] }} [filterOptions]
 */
export async function exportAveragePartUsagePerMonthToExcel(
  rows,
  filename = "average_part_usage_per_month.xlsx",
  sheetName = "Average Part Usage Per Month",
  filterOptions = {}
) {
  const headerLabels = [
    "Stock Code",
  ...
  const numericKeys = new Set(["unitcost", "qty_used", "totalcost", "costpermonth", "qtypermonth"]);
  const dataRows = (Array.isArray(rows) ? rows : []).map((row) =>
    accessorKeys.map((key) =>
      numericKeys.has(key) ? formatCost(row[key]) : (row[key] != null ? String(row[key]) : "")
    )
  );
  const headerRow = headerLabels;
  const colCount = headerRow.length;
  ...
  ws.getRow(1).getCell(1).value = "Average Part Usage Per Month";
  ws.getRow(1).getCell(1).font = { name: "Aptos", size: 18 };
  ws.getRow(1).getCell(1).alignment = { horizontal: "center", vertical: "middle" };
  ...
  const rightAlignCols = new Set([4, 5, 6, 7, 8]); // D, E, F, G, H
  for (let c = 1; c <= colCount; c++) {
    const headerCell = ws.getRow(headerExcelRow).getCell(c);
    headerCell.value = headerRow[c - 1] ?? "";
    headerCell.font = { name: "Aptos", size: 11, bold: true };
    ...
    if (rightAlignCols.has(c)) headerCell.alignment = { horizontal: "right" };
  }
  ...
  for (let r = headerExcelRow + 1; r <= lastDataExcelRow; r++) {
    for (let c = 1; c <= colCount; c++) {
      ...
      if (rightAlignCols.has(c)) cell.alignment = { horizontal: "right" };
    }
  }
...
/**
 * Export Part Usage As % of Jobs grid to Excel with formatting:
 * - A1: "Part Usage As % of Jobs", merge A1:G1, Aptos 18, centred
 * - Row 2: height 57; image top-left 96x96
 * - A3: "Report Date: " + current date, merge A3:G3
 * - A4-A8: From/To, Mine, Shaft, Section, Gang; A9: Equipment Types (merge A9:ZZ9)
 * - Rows 3-9: Aptos 14
 * - Tabular data starts on row 11; font Aptos 11; header bold + yellow; gridlines
 * @param {Array<Object>} rows - Grid data (stockcode, part, unitcost, qtyreplaced, totalcost, qty, percperjobs)
 * @param {{ fromDate?: string, toDate?: string, mine?: string, shaft?: string, section?: string, gang?: string, equipmentTypeLabels?: string[] }} [filterOptions]
 */
export async function exportPartUsagePercOfJobsToExcel(
  rows,
  filename = "part_usage_perc_of_jobs.xlsx",
  sheetName = "Part Usage As % of Jobs",
  filterOptions = {}
) {
  const headerLabels = [
    "Stock Code",
  ...
  const accessorKeys = ["stockcode", "part", "unitcost", "qtyreplaced", "totalcost", "qty", "percperjobs"];
  const intKeys = new Set(["qtyreplaced", "qty"]);
  const dataRows = (Array.isArray(rows) ? rows : []).map((row) =>
    accessorKeys.map((key) =>
      intKeys.has(key)
        ? formatNumInt(row[key])
        : key === "unitcost" || key === "totalcost" || key === "percperjobs"
          ? formatCost(row[key])
  ...
  ws.getRow(1).getCell(1).value = "Part Usage As % of Jobs";
  ws.getRow(1).getCell(1).font = { name: "Aptos", size: 18 };
  ws.getRow(1).getCell(1).alignment = { horizontal: "center", vertical: "middle" };
  ...
  const rightAlignCols = new Set([3, 4, 5, 6, 7]); // C, D, E, F, G (Unit Cost, Qty Replaced, Total Cost, Qty, Perc Per Jobs)
  for (let c = 1; c <= colCount; c++) {
    const headerCell = ws.getRow(headerExcelRow).getCell(c);
    headerCell.value = headerRow[c - 1] ?? "";
    headerCell.font = { name: "Aptos", size: 11, bold: true };
...
/**
 * Export Equipment Abuse report to Excel. Tabular data starts on row 11; font Aptos 11; header bold + yellow; gridlines.
 * @param {Array<Object>} rows - Grid data (mine, shaft, section, gang, type, serialno, jobno, datein, technician, stockcode, part, qty, unitcost, total)
 * @param {string} [filename]
 * @param {string} [sheetName]
 * @param {{ fromDate?: string, toDate?: string, mine?: string, shaft?: string, section?: string, gang?: string, equipmentTypeLabels?: string[] }} [filterOptions]
 */
export async function exportEquipmentAbuseToExcel(
  rows,
  filename = "equipment_abuse.xlsx",
  sheetName = "Equipment Abuse",
  filterOptions = {}
) {
  const headerLabels = [
    "Mine",
  ...
  const accessorKeys = ["mine", "shaft", "section", "gang", "type", "serialno", "jobno", "datein", "technician", "stockcode", "part", "qty", "unitcost", "total"];
  const qtyKey = "qty";
  const dateinKey = "datein";
  const dataRows = (Array.isArray(rows) ? rows : []).map((row) =>
    accessorKeys.map((key) =>
      key === qtyKey
        ? formatNumInt(row[key])
        : key === "unitcost" || key === "total"
          ? formatCost(row[key])
          : key === dateinKey
  ...
  // Merge to column N (14) for title and info rows
  const mergeEndColN = 14;

  ws.getRow(1).getCell(1).value = "Equipment Abuse";
  ws.getRow(1).getCell(1).font = { name: "Aptos", size: 18 };
  ws.getRow(1).getCell(1).alignment = { horizontal: "center", vertical: "middle" };
  ws.mergeCells(1, 1, 1, mergeEndColN);
  ...
  ws.getRow(3).getCell(1).value = `Report Date: ${reportDateStr}`;
  ws.getRow(3).getCell(1).font = infoFont;
  ws.mergeCells(3, 1, 3, mergeEndColN); // A3:N3

  ws.getRow(4).getCell(1).value = `From  ${fromDateStr}  To  ${toDateStr}`;
  ws.getRow(4).getCell(1).font = infoFont;
  ws.mergeCells(4, 1, 4, mergeEndColN); // A4:N4

  ws.getRow(5).getCell(1).value = `Mine: ${mineLabel}`;
  ws.getRow(5).getCell(1).font = infoFont;
  ws.mergeCells(5, 1, 5, mergeEndColN); // A5:N5

  ws.getRow(6).getCell(1).value = `Shaft: ${shaftLabel}`;
  ws.getRow(6).getCell(1).font = infoFont;
  ws.mergeCells(6, 1, 6, mergeEndColN); // A6:N6

  ws.getRow(7).getCell(1).value = `Section: ${sectionLabel}`;
  ws.getRow(7).getCell(1).font = infoFont;
  ws.mergeCells(7, 1, 7, mergeEndColN); // A7:N7

  ws.getRow(8).getCell(1).value = `Gang: ${gangLabel}`;
  ws.getRow(8).getCell(1).font = infoFont;
  ws.mergeCells(8, 1, 8, mergeEndColN); // A8:N8
  ...
  const rightAlignCols = new Set([12, 13, 14]); // Qty, Unit Cost, Total
  for (let c = 1; c <= colCount; c++) {
    const headerCell = ws.getRow(headerExcelRow).getCell(c);
    headerCell.value = headerRow[c - 1] ?? "";
    headerCell.font = { name: "Aptos", size: 11, bold: true };
...
/**
 * Export Monthly Cost Per Equipment Type report to Excel. Tabular data starts on row 11; font Aptos 11; header bold + yellow; gridlines.
 * @param {Array<Object>} rows - Grid data (equipmentcategory, equipmenttype, numberofjobs, totalcost, avgcostperitem, avgcostperitempermonth)
 * @param {string} [filename]
 * @param {string} [sheetName]
 * @param {{ fromDate?: string, toDate?: string, mine?: string, shaft?: string, section?: string, gang?: string, equipmentTypeLabels?: string[] }} [filterOptions]
 */
export async function exportMonthlyCostPerTypeToExcel(
  rows,
  filename = "monthly_cost_per_type.xlsx",
  sheetName = "Monthly Cost Per Equipment Type",
  filterOptions = {}
) {
  const headerLabels = [
    "Equipment Category",
    ...
    "Total Cost",
    "Avg Cost Per Item",
    "Avg Cost Per Item Per Month",
  ];
  const accessorKeys = ["equipmentcategory", "equipmenttype", "numberofjobs", "totalcost", "avgcostperitem", "avgcostperitempermonth"];
  const numberofjobsKey = "numberofjobs";
  const costKeys = new Set(["totalcost", "avgcostperitem", "avgcostperitempermonth"]);
  const dataRows = (Array.isArray(rows) ? rows : []).map((row) =>
    accessorKeys.map((key) =>
      key === numberofjobsKey
        ? formatNumInt(row[key])
  ...
  ws.getRow(1).getCell(1).value = "Monthly Cost Per Equipment Type";
  ws.getRow(1).getCell(1).font = { name: "Aptos", size: 18 };
  ws.getRow(1).getCell(1).alignment = { horizontal: "center", vertical: "middle" };
  ws.mergeCells(1, 1, 1, 6);
  ...
  const rightAlignCols = new Set([3, 4, 5, 6]); // Number of Jobs, Total Cost, Avg Cost Per Item, Avg Cost Per Item Per Month
  for (let c = 1; c <= colCount; c++) {
    const headerCell = ws.getRow(headerExcelRow).getCell(c);
    headerCell.value = headerRow[c - 1] ?? "";
    headerCell.font = { name: "Aptos", size: 11, bold: true };
...
/**
 * Export Cost Per Equipment Item report to Excel. Tabular data starts on row 11; font Aptos 11; header bold + yellow; gridlines.
 * @param {Array<Object>} rows - Grid data (type, serialno, pistonno, mine, shaft, section, gang, cost, monthlycost)
 * @param {string} [filename]
 * @param {string} [sheetName]
 * @param {{ fromDate?: string, toDate?: string, mine?: string, shaft?: string, section?: string, gang?: string, equipmentTypeLabels?: string[] }} [filterOptions]
 */
export async function exportCostPerItemToExcel(
  rows,
  filename = "cost_per_item.xlsx",
  sheetName = "Cost Per Equipment Item",
  filterOptions = {}
) {
  const headerLabels = [
    "Type",
  ...
  const accessorKeys = ["type", "serialno", "pistonno", "mine", "shaft", "section", "gang", "cost", "monthlycost"];
  const costKeys = new Set(["cost", "monthlycost"]);
  const dataRows = (Array.isArray(rows) ? rows : []).map((row) =>
    accessorKeys.map((key) =>
      costKeys.has(key)
        ? formatCost(row[key])
  ...
  ws.getRow(1).getCell(1).value = "Cost Per Equipment Item";
  ws.getRow(1).getCell(1).font = { name: "Aptos", size: 18 };
  ws.getRow(1).getCell(1).alignment = { horizontal: "center", vertical: "middle" };
  ...
  ws.getRow(7).getCell(1).value = `Section: ${sectionLabel}`;
  ws.getRow(7).getCell(1).font = infoFont;
  ws.mergeCells(7, 1, 7, 9);

  ws.getRow(8).getCell(1).value = `Gang: ${gangLabel}`;
  ws.getRow(8).getCell(1).font = infoFont;
  ws.mergeCells(8, 1, 8, 9);
  ...
  const rightAlignCols = new Set([8, 9]); // Cost, Monthly Cost
  for (let c = 1; c <= colCount; c++) {
    const headerCell = ws.getRow(headerExcelRow).getCell(c);
    headerCell.value = headerRow[c - 1] ?? "";
    headerCell.font = { name: "Aptos", size: 11, bold: true };
...
/**
 * Export Mean Time Between Failures report to Excel. Tabular data starts on row 11; font Aptos 11; header bold + yellow; gridlines.
 * @param {Array<Object>} rows - Grid data (type, serialno, mine, shaft, section, gang, days, failures, meandaysbetweenfailures)
 * @param {string} [filename]
 * @param {string} [sheetName]
 * @param {{ fromDate?: string, toDate?: string, mine?: string, shaft?: string, section?: string, gang?: string, equipmentTypeLabels?: string[] }} [filterOptions]
 */
export async function exportMeanTimeBetweenFailuresToExcel(
  rows,
  filename = "mean_time_between_failures.xlsx",
  sheetName = "Mean Time Between Failures",
  filterOptions = {}
) {
  const headerLabels = [
    "Type",
  ...
  const accessorKeys = ["type", "serialno", "mine", "shaft", "section", "gang", "days", "failures", "meandaysbetweenfailures"];
  const intKeys = new Set(["days", "failures", "meandaysbetweenfailures"]);
  const dataRows = (Array.isArray(rows) ? rows : []).map((row) =>
    accessorKeys.map((key) =>
      intKeys.has(key)
        ? formatNumInt(row[key])
  ...
  ws.getRow(1).getCell(1).value = "Mean Time Between Failures";
  ws.getRow(1).getCell(1).font = { name: "Aptos", size: 18 };
  ws.getRow(1).getCell(1).alignment = { horizontal: "center", vertical: "middle" };
  ws.mergeCells(1, 1, 1, mergeEndCol);
  ...
  const rightAlignCols = new Set([7, 8, 9]); // Days, Failures, Mean Days Between Failures
  for (let c = 1; c <= colCount; c++) {
    const headerCell = ws.getRow(headerExcelRow).getCell(c);
    headerCell.value = headerRow[c - 1] ?? "";
    headerCell.font = { name: "Aptos", size: 11, bold: true };
...
/**
 * Export Equipment Population By Area report to Excel. Tabular data starts on row 11; font Aptos 11; header bold + yellow; gridlines.
 * No date range (report has hideDateRange). Info rows: Report Date, Mine, Shaft, Section, Gang, Equipment Types.
 * @param {Array<Object>} rows - Grid data (category, type, count)
 * @param {string} [filename]
 * @param {string} [sheetName]
 * @param {{ mine?: string, shaft?: string, section?: string, gang?: string, equipmentTypeLabels?: string[] }} [filterOptions]
 */
export async function exportEquipmentPopulationByAreaToExcel(
  rows,
  filename = "equipment_population_by_area.xlsx",
  sheetName = "Equipment Population By Area",
  filterOptions = {}
) {
  const headerLabels = ["Category", "Type", "Count"];
  const accessorKeys = ["category", "type", "count"];
  const dataRows = (Array.isArray(rows) ? rows : []).map((row) =>
    accessorKeys.map((key) =>
      key === "count"
        ? formatNumInt(row[key])
  ...
  const headerRow = headerLabels;
  const colCount = headerRow.length;
  const mergeEndCol = 9; // A1:I1, A3:I3, etc.
  const headerExcelRow = 11;
  const lastDataExcelRow = headerExcelRow + dataRows.length;
  ...
  ws.getRow(1).getCell(1).value = "Equipment Population By Area";
  ws.getRow(1).getCell(1).font = { name: "Aptos", size: 18 };
  ws.getRow(1).getCell(1).alignment = { horizontal: "center", vertical: "middle" };
  ws.mergeCells(1, 1, 1, mergeEndCol);
  ...
  const rightAlignCols = new Set([3]); // Count
  for (let c = 1; c <= colCount; c++) {
    const headerCell = ws.getRow(headerExcelRow).getCell(c);
    headerCell.value = headerRow[c - 1] ?? "";
    headerCell.font = { name: "Aptos", size: 11, bold: true };
